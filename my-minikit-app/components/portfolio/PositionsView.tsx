...
export function PositionsView() {
    ...
    return (
      <Card>
        ...
        <div className="space-y-3">
          {[1, 2].map((i) => (
            <div key={i} className="animate-pulse p-3 rounded-lg bg-[var(--color-background)]">
              <div className="h-5 bg-[var(--color-border)] rounded w-24 mb-2" />
  ...
  return (
    <Card>
      ...
      {/* Account summary */}
      <div className="mb-4 p-3 rounded-lg bg-[var(--color-background)] grid grid-cols-2 gap-3">
        <div>
          <p className="text-xs text-[var(--color-text-muted)]">Account Value</p>
        ...
        <div className="space-y-3">
          {positions.map((position: Position) => {
            ...
            return (
              <div
                key={position.symbol}
                className="p-3 rounded-lg bg-[var(--color-background)] border-l-2"
                style={{
                  borderLeftColor: isLong
                    ? "var(--color-long)"
                    : "var(--color-short)",
                }}
              >
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div
                    className={`text-right ${pnl.isPositive ? "text-[var(--color-long)]" : pnl.isNegative ? "text-[var(--color-short)]" : "text-[var(--color-text-secondary)]"}`}
                  >
                ...
                <div className="grid grid-cols-3 gap-2 text-sm mb-3">
                  <div>
                    <p className="text-[var(--color-text-muted)] text-xs">Size</p>
                    <p className="font-mono text-[var(--color-text-primary)]">
                      {formatSize(position.size)}
                    </p>
                  </div>
                  <div>
                    <p className="text-[var(--color-text-muted)] text-xs">Entry</p>
                    <p className="font-mono text-[var(--color-text-primary)]">
                      ${formatPrice(position.entryPrice)}
                    </p>
                  </div>
                  <div>
                    <p className="text-[var(--color-text-muted)] text-xs">Mark</p>
                    <p className="font-mono text-[var(--color-text-primary)]">
